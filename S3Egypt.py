...
saData = data[data['Country/Region']=="Egypt"][['Confirmed','Deaths','Recovered']]
...
xtrain = cnsa.iloc[:117,:-2]
xtest = cnsa.iloc[117:,:-2]
ytrain = cnsa['CP'].iloc[:117]
ytest = cnsa['CP'].iloc[117:]

ytrainDP = cnsa['DP'].iloc[:117]
ytestDP = cnsa['DP'].iloc[117:]
...
DataTOPlotConf = data[data['Country/Region']=="Egypt"][['ObservationDate','Confirmed','Deaths']]
...
modelDP.compile(loss=ks.losses.mean_squared_error,optimizer=ks.optimizers.Adam())


modelDP.fit(mmsXtrain,mmsYDPtrain,epochs=20,batch_size=2,validation_data=(mmsXtest,mmsYDPtest))
...
#DataTOPlotConf.to_csv("EgyptDataVizData.csv")
