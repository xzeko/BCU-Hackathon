...
saData = data[data['Country/Region']=="South Sudan"][['Confirmed','Deaths','Recovered']]
...
xtrain = cnsa.iloc[:66,:-2]
xtest = cnsa.iloc[66:,:-2]
ytrain = cnsa['CP'].iloc[:66]
ytest = cnsa['CP'].iloc[66:]

ytrainDP = cnsa['DP'].iloc[:66]
ytestDP = cnsa['DP'].iloc[66:]
...
model.compile(loss=ks.losses.mean_absolute_percentage_error,optimizer=ks.optimizers.Adam())
...
DataTOPlotConf = data[data['Country/Region']=="South Sudan"][['ObservationDate','Confirmed','Deaths']]


DataTOPlotConf['testConf'] = DataTOPlotConf['Confirmed']
DataTOPlotConf['testConf'].iloc[-14:]  = ytest.reset_index()['CP'].values
...
modelDP.compile(loss=ks.losses.mean_absolute_percentage_error,optimizer=ks.optimizers.Adam())
...
DataTOPlotConf['testDeath'].iloc[-14:]  = ytestDP.reset_index()['DP'].values
...
#DataTOPlotConf.to_csv("SouthSudanDataVizData.csv")
